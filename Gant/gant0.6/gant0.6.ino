...
void sendRF(int quoi, int value) {
...
//valeurs du gant
  for (int i=0; i<8; i++) {
    if (debug_mode) {
      Serial.print("to_"); Serial.print(player); Serial.print("_from_:"); Serial.print(object); Serial.print("_valAdresse_:"); 
      Serial.print(i); Serial.print("_valeur_:"); Serial.println(valeursGant[i]);
    }

  
  //byte message[32] = "obj_:#gant1_reg_:x_val_:xxxxx";
  //messageStru.commande = i;
  //messageStru.valeur = valeursGant[i];

  message[17]= i + '0' ;
      if (valeursGant[i] < 0) {message[24]='-'; valeursGant[i] = -valeursGant[i] ; }// valeursGant[i];}
      else if (valeursGant[i] >= 0) {message[24]='+';}
      
      if (valeursGant[i] > 255) {valeursGant[i] = valeursGant[i] / 10;}
      //else if (valeursGant[i] < -255) {valeursGant[i] = valeursGant[i] / -10;}


    message[25]= valeursGant[i]; //byte(valeursGant[i]) ;  //derniere valeur
...
if (debug_mode) {Serial.print("cible_"); Serial.println(cible);}
  ...
  messageStru.commande = 'j';//Serial.read();
  messageStru.valeur = an;//Serial.parseInt();
  messageStru.objectStru = 'g' ;
    //Mirf.send(message); // On envoie le message
  Mirf.send((byte*) &message); // On envoie le message (Strings) et strucutre
