...
{ 
  ...
  Serial.begin(9600); 
  Serial.println("Epee de feu V0.62");
...
/*
  char msg2[17]= "from: {dreadbond}";
  vw_send((uint8_t *)msg2, 17); //strlen(msg) + x
  vw_wait_tx(); // Wait until the whole message is gone

  char msg3[13]= "objet: {epee}";
  vw_send((uint8_t *)msg3, 13); //strlen(msg) + x
  vw_wait_tx(); // Wait until the whole message is gone

  char msg4[16]= "action: {taille}";
  vw_send((uint8_t *)msg4, 16); //strlen(msg) + x
  vw_wait_tx(); // Wait until the whole message is gone

  char msg5[13]= "valeur: {520}";
//  msg5[10] = "520" ;
//  msg5[13]= "}";
  vw_send((uint8_t *)msg5, 13); //strlen(msg) + x
  vw_wait_tx(); // Wait until the whole message is gone
  */
  digitalWrite(13, LOW);
/*
