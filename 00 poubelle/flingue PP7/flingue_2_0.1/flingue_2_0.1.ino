...
Adafruit_NeoPixel stripCanon = Adafruit_NeoPixel(PixCanon, 3, NEO_GRB + NEO_KHZ800); //1. nuber defines number of pixels you have connected, 2. is number of the pin your data is connected to, 3 and 4 are other arguments but use these as they are default for most neopixels but refet to datasheet for other options
...
//détection Ir : faisage de message
  if (irrecv.decode(&results)) {
     ...
     if (char(results.value) == '!') {write_message = HIGH;}
        ...
        posCharRcv++ ;
        resetCible = millis() + 300;                    //délai avant perte de cible 300 à la base. 200 ensuite. 150 minimum
...
String recParamString=String(receiveBox.param);
  ...
  if (recToString == IDObject && process) {
    ...
    if (recParamString == "shootFB") {shootFb(0); tokenShoot=1; }
