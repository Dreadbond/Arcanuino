...
Serial.println(sortCast);
    if (sortCast > 10 && millis() > nextRefresh) {sortUpdate = HIGH; nextRefresh += 50;}

    //if (sortUpdate == HIGH) {Serial.print("891871871");}
      if (sortCast == 49 && sortUpdate) {feu_pattern();sortUpdate = LOW;}    //feu  
      if (sortCast == 50 && sortUpdate) {poison_pattern();sortUpdate = LOW;}    //vie
      if (sortCast == 51 && sortUpdate) {bouclier_pattern();sortUpdate = LOW;}    //bou
      if (sortCast == 52 && sortUpdate) {soins_pattern();sortUpdate = LOW;}    //bou
      if (sortCast == 53 && sortUpdate) {drain_de_vie_pattern();sortUpdate = LOW;}    //bou
      if (sortCast == 54 && sortUpdate) {super_punch_pattern();sortUpdate = LOW;}    //bou
    
      if (sortCast == 48 && sortUpdate) {offCast(); sortUpdate = LOW;}
      //sortUpdate = LOW;  }
      

      if (modeCom == 49) {strip.show();} //strip.show() bloque toute les réceptions. Il faut l'éteindre pour écouter. Conflit de lecture i2c ? même adresse ? même temps ?
