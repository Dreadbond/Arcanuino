...
void hubReceiveSP() {
  ...
  for (int i=0 ; i< thisHub.inMessage.length() ; i++) {
      //Serial.println("2");
    if (thisHub.inMessage.startsWith(":hub", i) ) {
      for (int j=0 ; j < thisHub.inMessage.length() ; j++) {
        if (thisHub.inMessage.startsWith("beenShot", j) ){
        thisHub.tokenImpact = 1 ;
        //Serial.println("3");
        break;
         }

        if (thisHub.inMessage.startsWith("health", j) ){
        thisHub.life = 50 ;
