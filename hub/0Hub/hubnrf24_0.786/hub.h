...
typedef struct{
  ...
  if (chrono>TC2 && chrono<TC3) {
      ...
      bright = map(chrono, TC2, TC3, 80, 0) ;
      bright /= 100 ;
  ...
  ////////////////////////////////////////////////////////////////////Shield shot
  bool tokenShieldShot ;
  void shieldShotFB(String cmd = ""){
    static bool effetStopFlag ;
    static unsigned long chrono ;
    static unsigned long timeTag ;
    static float red ;
    ...
    static int TC0 = 0 ;
    static int TC1 = TC0 + 50 ;
    static int TC2 = TC1 + 100 ;
    static int TC3 = TC2 + 250 ;
    long more = 2000 ;
    ...
    if (chrono>TC0 && chrono<TC1) {
      ...
      bright = map(chrono, TC0, TC1, 70, 100) ;
      bright /= 100 ;
  ...
  if (chrono>TC2 && chrono<TC3) {
      ...
      bright = map(chrono, TC2, TC3, 100, 70) ;
      bright /= 100 ;
...
void hubFeedback() {
  ...
  //{"to":":hub","from":"!111111","param":"shieldShot","value":1}
  ...
  for (int i=0 ; i< inMessage.length() ; i++) {
    if (inMessage.startsWith(":hub", i) ) {
      for (int j=0 ; j < inMessage.length() ; j++) {
          ...
          if (inMessage.startsWith("shieldShot", j) ){
          thisHub.tokenShieldShot = 1 ;
          break;
          }
...
void hubCode() {
  ...
  if (thisHub.tokenShieldShot) {
    thisHub.shieldShotFB() ;
    }
