...
void moreTime(int value=700){
  coolReset = millis() + value ;
  }
...
void movement2fb(int value = 1000) {
...
void movement2bisFb(int value = 1000) {
...
void clockWiseFb(int value = 1000) {
...
void antiClockWiseFb(int value = 1000) {
...
void moulinetFb(int value = 1000) {
...
if (to == ":sword" && justReceive) {
  ...
  if(paramam == "z") { sword.acc[2] = value ; }

  if(paramam == "capa") {sword.capa = value ; }
  }
