...
void playPatternOne(){              //a pattern function
...
//////////////////////////////////////////////////////////////////////And from this part, you can design whatever you want !
  ...
  if (chrono>TC0 && chrono<TC1) {                                          //"if" function to trigger events, just put the right timecodes. Here, it begins at 0, the first function's triggering, to TC1, the end of your soundfile
    ...
    index = map(chrono, TC0, TC1, 0, PIXNUMBER * 10 );       //map() function. So the index goes from 0 to your number of NP leds (PIXNUMBER) proportionally with your soundfile
    ...
    for (int j=0 ; j < index ; j++) {       //select NP from 0 to index, where j is the pixel number
      red   *= bright/100 ;
      green *= bright/100 ;
      blue  *= bright/100 ;
    ...
    int smoothingValue = 10 ;          //The beginning brightness of the Led in percent. 10 is good
                                       //Just because if the light begins at 0, it creates a harder effect
    ...
    red   *= remainingDim * bright / 100 ;   //The calculation to dim the last Led
  ...
  delay(5000);     //and delete this, it was just for the demo mode !
  }
}
...
void setup() 
{
  ...
  Serial.begin(9600); 
  Serial.println("Andrew' project V0.1");
...
void loop() {
  ...
  if (playToken)   { playPatternOne();}     //If there's a token in the system, we call a function to begin the Neopixel animation.
                                            ...
                                            //A stop function is planned, soon.
}
