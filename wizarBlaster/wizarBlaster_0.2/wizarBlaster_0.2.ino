...
void setup()
{
  ...
  Serial.println("WizarBlaster 0.2");
...
if (digitalRead(3) == LOW && reloadFlag == HIGH ) {
      tokenReload=HIGH;

      messageStru.commande = 'r' ;//Serial.read(); //r : recharge
      messageStru.valeur = 1 ;//Serial.parseInt(); //valeur : 1, ok. 2, non. ...
...
if (digitalRead(4) == LOW ) {
  ...
  modeFeu++;
    if(modeFeu>2) {modeFeu=1;}
  delay(2000);
  //debounceGachette = millis() + 20;
  Serial.println("Changing mode...");
  }

if (tokenReload) {reloadLedFB(); tokenFire=LOW; }
