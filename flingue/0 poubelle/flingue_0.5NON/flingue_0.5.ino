...
////////////////////////////////////Envoi action a : tir
if (trigger == LOW && trigger!= oldTrigger && panFlag == HIGH && millis() > debounceGachette) {  //1==1){
...
//////////////////////////////////////Feedback
  ...
  if (String(receiveBox.to).equals(IDObject) && process) {
      //Serial.println(receiveBox.to); Serial.println(receiveBox.param);
    if (String(receiveBox.param).equals("shootFB")) {tokenShoot=1;}
    if (String(receiveBox.param).equals("rldFB")) {tokenReload=1;}
    if (String(receiveBox.param).equals("sightFB")) {tokenSight=1;}
...
if (trigger == HIGH && millis() > debounceGachette)
  {
      if (!panFlag){
      strcpy( sendBox.param, "trigger");
      sendBox.value = 0;
